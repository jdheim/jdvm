...
setupFirefox() {
  cleanupBrokenSymlinks "/home/${USER}/.config/mozilla"
  local oldProfileDir profileDir
  oldProfileDir="$(printf '%s\n' "/home/${USER}/.mozilla/firefox"/*.dev)"
  profileDir="$(printf '%s\n' "/home/${USER}/.config/mozilla/firefox"/*.dev)"
  if [[ -e "/home/${USER}/.mozilla" && ! -e "/home/${USER}/.config/mozilla" ]]; then
    echo -e "${INFO} Migrated $(basename "${oldProfileDir}") Firefox profile to /home/${USER}/.config/mozilla"
    mv "/home/${USER}/.mozilla" "/home/${USER}/.config/mozilla"
    cleanupBrokenSymlinks "/home/${USER}/.config/mozilla"
    profileDir="$(printf '%s\n' "/home/${USER}/.config/mozilla/firefox"/*.dev)"
  fi
  if notExistsOrNotSymlink "${profileDir}/user.js"; then
    ...
    if [[ ! -e "${profileDir}" ]]; then
      ...
      if [[ -e "${profileDir}" ]]; then
        ...
        echo -e "${WARN} Could not create $(basename "${profileDir}") Firefox profile"
        return 0
      fi
    fi
