...
class VersionsTest extends JDVMContainer {
    ...
    @Test
    void testUbuntu() {
        String ubuntuVersion = getJDVMExecutor().run("grep \"VERSION_ID=\" \"/etc/os-release\" | cut -d '\"' -f2").exec();
    ...
    @Test
    void testVHS() {
        String vhsPath = getJDVMExecutor().executablePath("vhs").exec();
        assertThat(vhsPath).isEqualTo("/usr/bin/vhs");

        String vhsVersion = getJDVMExecutor().run("vhs --version | awk '{print $3}' | sed \"s/^v//\"").exec();
        assertThat(vhsVersion).isNotEmpty().isEqualTo(MAVEN.getProperty("vhs.version"));
    }
    ...
    @Test
    void testJdk() {
        CharSequence jdkLtsVersionWithDistribution = MAVEN.getProperty("jdk-lts.version") + "-" + MAVEN.getProperty(
                "jdk.distribution");
        CharSequence jdkStsVersionWithDistribution = MAVEN.getProperty("jdk-sts.version") + "-" + MAVEN.getProperty(
                "jdk.distribution");
        CharSequence jdkLtsLegacyVersionWithDistribution = MAVEN.getProperty("jdk-lts-legacy.version") + "-" + MAVEN.getProperty(
                "jdk.distribution");
        CharSequence[] dirs = new CharSequence[]{
        ...
        String jdkStsVersion = getJDVMExecutor().run("/opt/java/"
                + jdkStsVersionWithDistribution
        ...
        String jdkLtsLegacyVersion = getJDVMExecutor().run("/opt/java/"
                + jdkLtsLegacyVersionWithDistribution
                + "/bin/java --version | grep \"openjdk\" | sed \"s/openjdk \\([^ ]*\\) .*/\\1/\"").exec();
    ...
    void testUv() {
        ...
        String uvVersion = getJDVMExecutor().run("uv --version | awk '{print $2}'").exec();
    ...
    void testGDB() {
        ...
        assertThat(gdbVersion).isNotEmpty().matches("17\\.\\d+");
    }
    ...
    void testCMake() {
        ...
        assertThat(cmakeVersion).isNotEmpty().matches("4\\.\\d+\\.\\d+");
    }
    ...
    @Test
    void testDockerSecrets() {
        String dockerCliPluginsDir = getJDVMExecutor().ls("/usr/libexec/docker/cli-plugins").exec();
        assertThat(dockerCliPluginsDir).contains("docker-pass");

        String dockerSecretsVersion = getJDVMExecutor().run(
                "docker pass version | grep -i \"version\" | awk '{print $2}' | sed \"s/^v//\"").exec();
        assertThat(dockerSecretsVersion).isNotEmpty().isEqualTo(MAVEN.getProperty("docker-pass.version"));
    }
    ...
    @Test
    void testGrype() {
        String grypePath = getJDVMExecutor().executablePath("grype").exec();
        assertThat(grypePath).isEqualTo("/usr/local/bin/grype");

        String grypeVersion = getJDVMExecutor().run("grype --version | awk '{print $2}'").exec();
        assertThat(grypeVersion).isNotEmpty().isEqualTo(MAVEN.getProperty("grype.version"));
    }
    ...
    @Test
    void testSyft() {
        String syftPath = getJDVMExecutor().executablePath("syft").exec();
        assertThat(syftPath).isEqualTo("/usr/local/bin/syft");

        String syftVersion = getJDVMExecutor().run("syft --version | awk '{print $2}'").exec();
        assertThat(syftVersion).isNotEmpty().isEqualTo(MAVEN.getProperty("syft.version"));
    }
    ...
    @Test
    void testK9s() {
        String k9sPath = getJDVMExecutor().executablePath("k9s").exec();
        assertThat(k9sPath).isEqualTo("/usr/local/bin/k9s");

        String bashCompletionDir = getJDVMExecutor().ls("/etc/bash_completion.d").exec();
        assertThat(bashCompletionDir).contains("k9s");

        String k9sVersion = getJDVMExecutor().run("k9s version --short | grep -i \"version\" | awk '{print $2}' | sed \"s/^v//\"")
                .exec();
        assertThat(k9sVersion).isNotEmpty().isEqualTo(MAVEN.getProperty("k9s.version"));
    }
