...
class DockerEntrypointTest extends JDVMContainer {
    ...
    @Test
    void dockerEntrypointInitialization() {
        String dockerEntrypointLog = runDockerEntrypoint();
        assertThat(dockerEntrypointLog).contains("[STEP] ----- Start initialization -----")
                .contains("[INFO] Running /docker-entrypoint.d/01-root-fix-user-volume-permissions.sh")
                .contains("[INFO] Running /docker-entrypoint.d/02-root-fix-x11-permissions.sh")
                .contains("[INFO] Running /docker-entrypoint.d/03-setup-user-home.sh")
                .contains("[INFO] Running /docker-entrypoint.d/04-sync-user-templates.sh")
                .contains("[INFO] Running /docker-entrypoint.d/05-sync-skeletal-user-templates.sh")
                .contains("[INFO] Running /docker-entrypoint.d/06-sync-local-templates.sh")
                .contains("[INFO] Running /docker-entrypoint.d/07-init-oh-my-zsh.sh")
                .contains("[INFO] Running /docker-entrypoint.d/08-init-jmeter.sh")
                .contains("[INFO] Running /docker-entrypoint.d/09-save-docker-env-variables.sh")
                .doesNotContain("[INFO] Running /docker-entrypoint.d/10")
                .contains("[SUCCESS] Initialization complete")
                .doesNotContain("[STEP] ----- Start systemd -----");
    }

    private String runDockerEntrypoint() {
    ...
    @Test
    void testFirefox() {
        String firefoxProfile = getJDVMExecutor().run(
                "ls \"%s/.config/mozilla/firefox\" | grep \"%s\"".formatted(USER_HOME, IMAGE_USER)).exec();
        String firefoxProfilePath = "%s/.config/mozilla/firefox/%s".formatted(USER_HOME, firefoxProfile);
    ...
    @Test
    void testKubectlKrewIndexBackup() {
